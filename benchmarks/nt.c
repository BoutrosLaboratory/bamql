...
uint32_t
compute_mapped_end (bam1_t * read)
{
      ...
      if (read->core.n_cigar == 0)
	{
	  mapped_position =
	    bam_is_rev (read) ? (compute_mapped_end (read) -
				 position) : (position - read->core.pos - 1);
	}
      else
	{
	  ...
	  for (int index = 0; index < read->core.n_cigar; index++)
	    {
	      uint8_t consume =
		bam_cigar_type (bam_cigar_op (bam_get_cigar (read)[index]));
	      uint32_t op_length =
		bam_cigar_oplen (bam_get_cigar (read)[index]);
	      for (int incr = 0; incr < op_length && required_offset >= 0;
		   incr++)
		{
		  if (consume & 1)
		    mapped_position++;
